...
impl Config {
    ...
    fn log_summary(&self) {
        ...
        info!("  Database:");
        info!("    Pool Size: {}-{}", self.database.min_connections, self.database.max_connections);
        info!("  API:");
...
impl BlockchainConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            ...
            start_block: get_env_parsed("START_BLOCK")?,
            poll_interval: Duration::from_millis(
                get_env_or("POLL_INTERVAL_MS", "2000").parse().unwrap_or(2000),
            ),
            batch_size: get_env_or("BLOCK_BATCH_SIZE", "1000").parse().unwrap_or(1000),
            max_retries: get_env_or("RPC_MAX_RETRIES", "3").parse().unwrap_or(3),
            retry_delay: Duration::from_millis(
                get_env_or("RPC_RETRY_DELAY_MS", "1000").parse().unwrap_or(1000),
            ),
        })
    }
}
        ...
        Ok(Self {
            ...
            producer: KafkaProducerConfig {
                message_timeout: Duration::from_millis(
                    get_env_or("KAFKA_MESSAGE_TIMEOUT_MS", "5000").parse().unwrap_or(5000),
                ),
                max_message_bytes: get_env_or("KAFKA_MAX_MESSAGE_BYTES", "20971520")
                    .parse()
                    .unwrap_or(20 * 1024 * 1024),
                batch_size: get_env_or("KAFKA_BATCH_SIZE", "16384").parse().unwrap_or(16384),
                ...
                idempotent: get_env_or("KAFKA_IDEMPOTENT", "true").parse().unwrap_or(true),
            },
        })
    }
}
    ...
    fn from_env() -> Result<Self> {
        ...
        Ok(Self {
            ...
            connect_timeout: Duration::from_secs(
                get_env_or("DB_CONNECT_TIMEOUT_SECS", "30").parse().unwrap_or(30),
            ),
            idle_timeout: Duration::from_secs(
                get_env_or("DB_IDLE_TIMEOUT_SECS", "600").parse().unwrap_or(600),
            ),
            max_lifetime: Duration::from_secs(
                get_env_or("DB_MAX_LIFETIME_SECS", "3600").parse().unwrap_or(3600),
            ),
    ...
    fn from_env() -> Result<Self> {
        Ok(Self {
            ...
            request_timeout: Duration::from_secs(
                get_env_or("API_REQUEST_TIMEOUT_SECS", "30").parse().unwrap_or(30),
            ),
            max_body_size: get_env_or("API_MAX_BODY_SIZE", "10485760")
                .parse()
                .unwrap_or(10 * 1024 * 1024),
            cors_enabled: get_env_or("API_CORS_ENABLED", "true").parse().unwrap_or(true),
...
impl ContractAddresses {
    fn from_env() -> Result<Self> {
        ...
        let friends_addr = std::env::var("THERA_FRIEND_ADDRESS")
            ...
            .map_err(|_| Error::MissingEnvVar { var: "THERA_FRIEND_ADDRESS / FRIENDS_ADDRESS" })?;

        // THERA_SOCIAL_ADDRESS is deprecated; fall back to friends address if unset
        let social_addr = std::env::var("THERA_SOCIAL_ADDRESS").unwrap_or_else(|_| friends_addr.clone());
    ...
    fn from_env() -> Result<Self> {
        Ok(Self {
            cache_ttl: Duration::from_secs(
                get_env_or("REC_CACHE_TTL_SECS", "300").parse().unwrap_or(300),
            ),
            max_candidates: get_env_or("REC_MAX_CANDIDATES", "1000").parse().unwrap_or(1000),
            min_score: get_env_or("REC_MIN_SCORE", "0.1").parse().unwrap_or(0.1),
            diversity_factor: get_env_or("REC_DIVERSITY_FACTOR", "0.2").parse().unwrap_or(0.2),
            trending_update_interval: Duration::from_secs(
                get_env_or("REC_TRENDING_UPDATE_SECS", "3600").parse().unwrap_or(3600),
            ),
            engagement_update_interval: Duration::from_secs(
                get_env_or("REC_ENGAGEMENT_UPDATE_SECS", "3600").parse().unwrap_or(3600),
            ),
